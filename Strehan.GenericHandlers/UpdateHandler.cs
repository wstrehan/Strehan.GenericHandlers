...
namespace Strehan.GenericHandlers
{
    ...
    /// </summary>
    /// <typeparam name="TID">Type of the ID used to identify the record to be updated</typeparam>
    /// <typeparam name="T">Type that defines the fields that are used for the update not including the Id</typeparam>
    /// <typeparam name="B">Type that defines the fields that are read back when getting a record by Id</typeparam>
    public class UpdateHandler<TID, T, B> : IHttpHandler
    {
        /// <summary>
        ...
        /// <summary>
        /// Stored procedure to be called with input parameters that match up with Type T with the addition of an Id parameter to identify which row to update
        /// </summary>
        public string UpdateStoredProcedureName { get; set; }
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                if (UpdateStoredProcedureName == string.Empty)
                {
                    throw new ArgumentException("Property UpdateStoredProcedureName must be set in handler");
                }
                ...
                dataAccess.Update(id, obj, UpdateStoredProcedureName);
